...
router.get('/', requireAuth, async (req, res) => {
  ...
  const result = await db.query(
    `SELECT cc.id, cc.body, cc.created_at, u.nick AS who, u.city
     FROM country_chat_messages cc JOIN users u ON u.id = cc.from_user_id
     WHERE cc.country = $1
     ORDER BY cc.created_at DESC LIMIT $2`,
    [country, limit]
  );
  const messages = result.rows.reverse().map((m) => ({ ...m, region: getRegion(country, m.city) }));
  res.json({ messages });
});

// ============================================
// POST /api/country-chat  { country, body }
// ============================================
router.post('/', requireAuth, async (req, res) => {
  const { country, body } = req.body;
  if (!ALLOWED_COUNTRIES.includes(country)) {
    return res.status(400).json({ error: 'Nepoznata država.' });
  }
  ...
  await db.query(
    'INSERT INTO country_chat_messages (country, from_user_id, body) VALUES ($1, $2, $3)',
    [country, req.user.userId, text]
  );
  res.json({ ok: true });
});
